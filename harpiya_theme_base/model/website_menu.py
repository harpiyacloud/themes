...
class WebsiteMenu(models.Model):
    ...
    is_dynamic_menu = fields.Boolean(string="Dinamik Menü", default=False)
    menu_label_text = fields.Char(string="Menü Etiket Metni", default=False,
                                  help="Menü bağlantısında görüntülenecek menü etiketi metni", translate=True)
    menu_label_text_color = fields.Char(string="Menü Etiketi Metin Rengi", default=False)
    ...
    def get_tree(self, website_id, menu_id=None):
        ...
        def make_tree(node):
            is_homepage = bool(
                node.page_id and self.env['website'].browse(website_id).homepage_id.id == node.page_id.id)
            menu_node = {
                'fields': {
                    'id': node.id,
                    'name': node.name,
