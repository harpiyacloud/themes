...
class HarpiyaThemeBase(http.Controller):
        ...
        if request.httprequest.method == 'POST':
            ...
            try:
                ...
                if e.args == harpiya.exceptions.AccessDenied().args:
                    values['error'] = _("Hatalı Kullanıcı Adı yada Şifre")
                else:
                    values['error'] = e.args[0]
        else:
            if 'error' in request.params and request.params.get('error') == 'access':
                values['error'] = _('Bu veritabanına yalnızca yetkililer erişebilir. Lütfen yöneticiye başvurun.')
    ...
    def web_auth_signup(self, *args, **kw):
        ...
        if 'error' not in qcontext and request.httprequest.method == 'POST':
            try:
                ...
                if not values:
                    result.update({'is_success': False, 'error': 'Form düzgün doldurulmamış'})
                    return result
                if values.get('password') != qcontext.get('confirm_password'):
                    result.update({'is_success': False, 'error': 'Şifreler uyuşmuyor; lütfen tekrar yazın.'})
                    return result

                if request.env["res.users"].sudo().search([("login", "=", qcontext.get("login"))]):
                    result.update(
                        {'is_success': False, 'error': 'Girdiğiniz e-posta adresi başkası tarafından kullanılıyor.'})
                    return result
                ...
                if not uid:
                    result.update({'is_success': False, 'error': 'Kimlik doğrulama başarısız oldu.'
                                   })
                    return result
                ...
                result.update({'is_success': False, 'error': 'Yeni hesap oluşturulamadı.'})
                return result
    ...
    def get_quick_view_item(self, product_id=None):
        ...
        if product_id:
            ...
            response = http.Response(template="harpiya_theme_base.quick_view_container", qcontext=values)
    ...
    def get_brand_slider_data(self):
        ...
        """
        response = http.Response(template="harpiya_theme_base.brand_slider_container")
        return response.render()
    ...
    def get_category_slider_data(self):
        ...
        """
        response = http.Response(template="harpiya_theme_base.category_slider_container")
        return response.render()
    ...
    def slider_preview(self, rec_id, **kwargs):
        ...
        :return:
        """
        return http.Response(template="harpiya_theme_base.slider_preview",
    ...
    def get_ajax_cart_item(self, product_id=None):
        ...
        if product_id:
            ...
            response = http.Response(template="harpiya_theme_base.ajax_cart_container", qcontext=values)
            return response.render()
    ...
    def get_ajax_cart_sucess(self, product_id=None):
        ...
        if product_id:
            ...
            response = http.Response(template="harpiya_theme_base.ajax_cart_success_container", qcontext=values)
            return response.render()
...
class HarpiyaThemeBaseExtended(WebsiteSaleWishlist):
    ...
    def _get_search_domain(self, search, category, attrib_values, search_in_description=True):
        ...
        if category_offers:
            pricelist_items = HarpiyaThemeBase.get_current_priclist_items_ids(self)
            if pricelist_items:
                ids = []
                products = []
                for item in pricelist_items:
        ...
        domain = super(HarpiyaThemeBaseExtended, self)._get_search_domain(search=search, category=category,
                                                                         attrib_values=attrib_values,
